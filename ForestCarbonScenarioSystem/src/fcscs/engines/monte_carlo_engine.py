...
def run_monte_carlo(event_tables, config):
    rng = np.random.default_rng(config.base_seed + 200)
    #1用历史数据训练模型 2蒙特卡洛预测
    model_context = prepare_context(config, event_tables)

    baseline_agbd_surface = model_context["baseline_prediction_surface"]
    agbd_stack = run_simulations(config, event_tables, model_context, rng)

    summary = build_summary(config, baseline_agbd_surface, agbd_stack, model_context)
    event_summaries = []
    for table in event_tables:
        event_summaries.append(table.summary())
    yearly_event_df = build_yearly_event_df(event_tables)
    output_files = write_raster_outputs(config, agbd_stack, model_context)
...
def run_simulations(config, event_tables, model_context, rng):
    ...
    for sim_index in range(config.mc_n_simulations):
        simulation_surface = run_single_simulation(
            config, event_tables, model_context, rng
        )
...
def run_single_simulation(config, event_tables, model_context, rng):
    climate_shift = float(rng.normal(0.0, 1.4))
    climate_noise = rng.normal(
        0.0, 1.8, size=(config.grid_rows, config.grid_cols)
    ).astype(np.float32)

    baseline_surface = predict_baseline_surface(
        config,
    ...
    for event_table in event_tables:
        ...
        rows, cols, predicted = predict_event_values(
            config,
            event_table,
            model_context["surfaces"],
            model_item["model"],
...
def build_summary(config, baseline_agbd_surface, agbd_stack, model_context):
    ...
    return {
        ...
        "n_simulations": config.mc_n_simulations,
        ...
        "ml_model_count": len(training_summary_df),
        "data_mode": "raster",
...
def build_yearly_event_df(event_tables):
    frames = []
    for table in event_tables:
...
def write_raster_outputs(config, agbd_stack, model_context):
    output_files = {}
...
def prepare_context(config, event_tables):
    rng = np.random.default_rng(config.base_seed + 150)
    surfaces = build_raster_feature_surfaces(config, rng)

    models, training_sample_df = train_models(config, event_tables, surfaces, rng)
    training_summary_df = build_training_summary_df(models)
    baseline_prediction_surface = predict_baseline_surface(
        config, surfaces, models["baseline"]["model"]
    )
...
def predict_baseline_surface(config, surfaces, model, climate_shift=0.0):
    rows = config.grid_rows
    cols = config.grid_cols
    span = config.target_year - config.base_year
    env_names = surfaces.get(
        ...
        for col in range(cols):
            ...
            for env_name in env_names:
                ...
                if env_name == "moisture":
                    value = clip_moisture(value + climate_shift * 0.03)
                feature_matrix[index, feature_col] = value
                feature_col += 1
            feature_matrix[index, feature_col] = span
            index += 1

    feature_matrix = fill_nan_matrix(feature_matrix)
...
def predict_event_values(config, event_table, surfaces, model, rng, climate_shift):
    ...
    feature_matrix = build_event_feature_matrix(
        config, records, surfaces, rng, climate_shift
    )
    feature_matrix = fill_nan_matrix(feature_matrix)
    predicted = model.predict(feature_matrix).astype(np.float32)
    predicted = np.maximum(predicted, 0.0)
...
def train_models(config, event_tables, surfaces, rng):
    if not history_training_ready(config):
        raise ValueError(
            "历史训练数据不足：请提供至少两个共同年份的AGBD、树冠覆盖度和土地利用栅格。"
        )

    history_result = train_models_from_history(config, event_tables, surfaces, rng)
    if history_result is not None:
        return history_result
    raise ValueError(
        "历史训练样本为空：请检查历史AGBD、树冠覆盖度、土地利用和森林损失驱动因素栅格。"
    )


def history_training_ready(config):
    ...
    common_years = sorted(
        set(agbd_paths.keys()) & set(tcc_paths.keys()) & set(lulc_paths.keys())
    )
    if len(common_years) < 2:
        return False
    for index in range(len(common_years) - 1):
        if common_years[index + 1] - common_years[index] == 1:
            return True
    return False


def train_models_from_history(config, event_tables, surfaces, rng):
    history = load_history_rasters(config)
    if history is None:
        return None

    baseline_df = build_history_baseline_training_df(config, surfaces, history, rng)
    if baseline_df.empty:
        return None

    baseline_features = baseline_feature_columns(surfaces)
    event_features = event_feature_columns(surfaces)

    models = {}
    models["baseline"] = fit_model(
        "baseline", baseline_df, baseline_features, config, config.base_seed + 701
    )

    logging_df = build_history_event_training_df(
        config, surfaces, history, "logging", rng
    )
    if logging_df.empty:
        if future_event_has_rows(event_tables, "logging"):
            raise ValueError(
                "历史采伐训练样本为空：请检查Drivers分类、loss year波段、历史TCC和历史AGBD。"
            )
    else:
        models["logging"] = fit_model(
            "logging", logging_df, event_features, config, config.base_seed + 702
        )

    edge_df = build_history_event_training_df(
        config, surfaces, history, "urban_edge", rng
    )
    if edge_df.empty:
        if future_event_has_rows(event_tables, "urban_edge"):
            raise ValueError(
                "历史城镇边缘扰动训练样本为空：请检查历史土地利用和树冠覆盖度。"
            )
    else:
        models["urban_edge"] = fit_model(
            "urban_edge", edge_df, event_features, config, config.base_seed + 703
        )

    conv_df = build_history_event_training_df(
        config, surfaces, history, "urban_conv", rng
    )
    if conv_df.empty:
        if future_event_has_rows(event_tables, "urban_conv"):
            raise ValueError(
                "历史城镇转换训练样本为空：请检查历史土地利用和树冠覆盖度。"
            )
    else:
        models["urban_conv"] = fit_model(
            "urban_conv", conv_df, event_features, config, config.base_seed + 704
        )

    training_sample_df = build_training_sample_df(
        baseline_df, logging_df, edge_df, conv_df
    )
    return models, training_sample_df


def future_event_has_rows(event_tables, event_type):
    for event_table in event_tables:
...
def load_history_rasters(config):
    agbd_paths = deconstruct_years(config.history_agbd_paths)
    tcc_paths = deconstruct_years(config.history_tcc_paths)
    lulc_paths = deconstruct_years(config.history_lulc_paths)
    ...
    for year in common_years:
        ...
        if not construct_path(lulc_paths[year]).exists():
            continue
        agbd, ul = read_grid(agbd_paths[year], to_float=True)
        tcc, ul = read_grid(tcc_paths[year], to_float=True)
        lulc, ul = read_grid(lulc_paths[year])
        tcc = normalize_percent_surface(tcc)
        if reference_shape is None:
            reference_shape = agbd.shape
        if (
            agbd.shape != reference_shape
            or tcc.shape != reference_shape
            or lulc.shape != reference_shape
        ):
            continue
    ...
    years = sorted(
        set(agbd_by_year.keys()) & set(tcc_by_year.keys()) & set(lulc_by_year.keys())
    )
    if len(years) < 2:
        return None
    ...
    if config.use_driver_sample_weight:
        logging_probability = read_driver_probability(
            config, config.logging_probability_band, reference_shape
        )
        urban_probability = read_driver_probability(
            config, config.urban_probability_band, reference_shape
        )
...
def read_driver_probability(config, band, shape):
    try:
        data, ul = read_band(config.drivers_raster_path, band, to_float=True)
...
def build_history_baseline_training_df(config, surfaces, history, rng):
    rows = []
    year_pairs = build_history_year_pairs(history["years"])
    forest_codes = deconstruct_codes(config.forest_lulc_codes, [1, 2, 3, 4, 5])
    max_per_pair = max(20, config.ml_sample_count // max(len(year_pairs), 1))
    ...
    for start_year, end_year in year_pairs:
        ...
        valid_mask = (
            np.isfinite(agbd_pre) & np.isfinite(agbd_end) & np.isfinite(tcc_pre)
        )
        ...
        for cell_id in cell_ids:
            ...
            add_env_values_to_record(record, surfaces, row, col)
            rows.append(record)
...
def build_history_event_training_df(config, surfaces, history, event_type, rng):
    ...
    for start_year, end_year in year_pairs:
        mask = build_history_event_mask(
            config, history, event_type, start_year, end_year, forest_codes, urban_codes
        )
        ...
        for cell_id in cell_ids:
            row = int(cell_id // config.grid_cols)
            col = int(cell_id % config.grid_cols)
            if not np.isfinite(agbd_pre[row, col]) or not np.isfinite(
                agbd_end[row, col]
            ):
                continue
...
def build_history_year_pairs(years):
    pairs = []
    for index in range(len(years) - 1):
        pairs.append((years[index], years[index + 1]))
    return pairs


def build_history_event_mask(
    config, history, event_type, start_year, end_year, forest_codes, urban_codes
):
    agbd_pre = history["agbd"][start_year]
    agbd_end = history["agbd"][end_year]
    ...
    if event_type == "logging":
        ...
        if drivers_class is None:
            return None
        mask = drivers_class == config.logging_driver_value
    ...
    edge_mask = build_edge_mask_from_conversion(
        conv_mask, np.isin(lulc_end, forest_codes)
    )
    return edge_mask & valid_mask


def build_edge_mask_from_conversion(conv_mask, forest_mask):
...
def build_training_sample_df(baseline_df, logging_df, urban_edge_df, urban_conv_df):
    rows = []
    max_rows = 30
    sample_groups = [
...
def baseline_feature_columns(surfaces):
    columns = ["AGBD_pre", "TCC_pre"]
    for name in surfaces.get(
        "env_feature_names", ["slope", "moisture", "accessibility"]
    ):
        columns.append(name)
    columns.append("span")
...
def event_feature_columns(surfaces):
    columns = ["AGBD_pre", "TCC_pre"]
    for name in surfaces.get(
        "env_feature_names", ["slope", "moisture", "accessibility"]
    ):
...
def fit_model(model_name, training_df, feature_columns, config, seed):
    training_df = clean_training_df(training_df, feature_columns)
    ...
    train_df, test_df = split_train_test(training_df, seed)
    ...
    x_train = fill_nan_matrix(x_train)
    x_test = fill_nan_matrix(x_test)
...
def clean_training_df(training_df, feature_columns):
...
def split_train_test(training_df, seed):
...
def build_training_summary_df(models):
    rows = []
...
def build_raster_feature_surfaces(config, rng):
    check_raster_inputs_for_prediction(config)
    agbd_pre, profile = read_grid(config.agbd_raster_path, to_float=True)
    tcc_pre, ul = read_grid(config.tcc_raster_path, to_float=True)
    ensure_min_valid_pixels(agbd_pre, "AGBD")
    ensure_min_valid_pixels(tcc_pre, "TCC")
    tcc_pre = normalize_percent_surface(tcc_pre)
    ...
    env_map = load_env_rasters(config, agbd_pre.shape)
    slope = get_required_surface(
        env_map, ["slope", "Slope", "坡度", "地形"], "地形因子"
    )
    moisture = get_required_surface(
        env_map, ["moisture", "MAP", "AET", "水分", "降水"], "气候水分因子"
    )
    accessibility = get_required_surface(
        env_map,
        ["accessibility", "DistRoadNet", "distance", "可达性", "道路", "人为"],
        "人为活动因子",
    )
    slope = normalize_slope_surface(slope)
    moisture = normalize_unit_surface(moisture, invert=False)
    accessibility = normalize_unit_surface(accessibility, invert=True)
    ...
    surfaces = {
        ...
        "raster_out_dir": construct_raster_dir(config),
    }
    env_feature_names = ["slope", "moisture", "accessibility"]
    ...
    for name, path_in in deconstruct_envs(config.env_raster_paths):
        ...
        if name not in env_map:
            continue
        extra_surface = normalize_unit_surface(env_map[name], invert=False)
...
def get_required_surface(env_map, candidate_names, label):
...
def normalize_percent_surface(surface):
...
def normalize_slope_surface(surface):
    result = surface.astype(np.float32).copy()
    result = np.clip(result, 0.0, 80.0)
    return result


def normalize_unit_surface(surface, invert=False):
    result = surface.astype(np.float32).copy()
    max_value = np.nanmax(result)
...
def check_raster_inputs_for_prediction(config):
    check_rasterAndalign(
        [
...
def construct_raster_dir(config):
...
def clip_moisture(value):
    value = float(np.clip(value, 0.10, 0.95))
    return value


def ensure_min_valid_pixels(surface, name, minimum=5):
    valid_count = int(np.isfinite(surface).sum())
    required = min(surface.size, minimum)
    if valid_count < required:
        raise ValueError(
            f"{name} 有效像元不足：至少需要 {required} 个，当前 {valid_count} 个。"
        )


def add_env_values_to_record(record, surfaces, row, col):
    for env_name in surfaces.get(
        "env_feature_names", ["slope", "moisture", "accessibility"]
    ):
...
def build_event_feature_matrix(config, records, surfaces, rng, climate_shift):
    ...
    severity_values = sample_prediction_severity(records, rng)
    tau_values, gap_values = sample_prediction_time_values(config, records, rng)
    ...
    for index in range(sample_count):
        ...
        feature_matrix[index, feature_col + 3] = float(
            records.iloc[index].get("patch_size", 1)
        )

    return feature_matrix


def sample_prediction_severity(records, rng):
...
def sample_prediction_time_values(config, records, rng):
    tau_values = []

    for _, record in records.iterrows():
        year = int(record.get("y_event", config.base_year))
        tau = max(config.target_year - year, 0)
...
def build_report(bundle):
    ...
    total_distribution_df = build_total_distribution_df(bundle)
...
def build_total_distribution_df(bundle):
    ...
    totals = np.nansum(
        stack * float(bundle.summary.get("pixel_area_ha", 1.0)), axis=(1, 2)
    )
    data = pd.DataFrame(
        {
            ...
            "total_agc": totals.astype(float)
            * float(bundle.summary.get("agbd_to_agc_factor", 0.47)),
        }
    )
    return data
